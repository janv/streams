...
A channel consists of a subscribe method (a stream), an optional update
function, and an optional synchronous value, that can either be the current value
or a sensible default while the consumer waits for the first value from the stream.
...
The reason we have the `value` is that often such an initial value is
...
If a initial value can be provided synchronously, we don't need this extra
...
channel with a default value, but don't provide a way to update the source.
