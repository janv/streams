...
describe("makeSource", () => {
  ...
  test("onSubscribe is called after adding the new handler", () => {
    const onSubscribe = vi.fn(() => expect(source.handlers.size).toBe(1));
  ...
  test("onUnsubscribe is called after removing a handler", () => {
    const onUnsubscribe = vi.fn(() => expect(source.handlers.size).toBe(0));
...
function testOperatorProperties<I, O = I>(operator?: Operator<I, O>) {
  ...
  describe("behaves like an operator", () => {
    ...
    maybeTest("forwards subscriptions and unsubscriptions", () => {
      ...
      const stream = vi.fn(() => unsubscribe);
...
describe("mapAsync", () => {
  test("unwraps promises in source and mapFn", () =>
  ...
  test("works with regular values", () =>
    new Promise<void>((done) => {
  ...
  test("does not emit or call mapFn when handler is unsubscribed", async () => {
    ...
    const mapFn = vi.fn((n: number) => mapPromise);
...
describe("kickoff", () => {
  ...
  test("does not use kickoff value when sync value coming from source", () => {
    ...
    const handler = vi.fn();
    const startWithProvider = vi.fn(() => 123);
...
describe("memo", () => {
  ...
  test("works with two subscribers", () => {
    const stream = makeSource<number>();
    const handler1 = vi.fn().mockName("handler1");
    const handler2 = vi.fn().mockName("handler2");
    const memoStream = memo()(stream);
    memoStream(handler1);
    memoStream(handler2);
...
describe("merge", () => {
  test("subscribes to all on subscription", () => {
    const onSubscribeA = vi.fn();
    const onSubscribeB = vi.fn();
  ...
  test("unsubscribes from all on unsubscribe", () => {
    const onUnsubscribeA = vi.fn();
    const onUnsubscribeB = vi.fn();
